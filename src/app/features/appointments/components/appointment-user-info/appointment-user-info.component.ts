...
export class AppointmentUserInfoComponent implements OnInit {
  ...
  @Input({ required: true }) userRoleToShow!: 'client' | 'specialist';
  ...
  get title(): string {
    return this.userRoleToShow === 'client' ? 'Cliente' : 'Especialista';
  }
  ...
  get customData() {
    ...
    if (this.userRoleToShow === 'client') {
      ...
      return [
        ...
        {
          label: 'Altura: ',
          value: client.height ? `${client.height} cm.` : 'Sin especificar',
        },
        {
          label: 'Peso: ',
          value: client.weight ? `${client.weight} kg.` : 'Sin especificar',
        }
      ];
