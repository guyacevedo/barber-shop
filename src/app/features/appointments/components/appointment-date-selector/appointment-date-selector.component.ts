...
export class AppointmentDateSelectorComponent {
  ...
  public async onDaySelected(day: Date): Promise<void> {
    ...
    if (availabilityForDay) {
      ...
      const clientId = this.authFacade.user()?.id;
      if (!clientId) {
      ...
      const [specialistAppointments, clientAppointments] = await Promise.all([
        this.appointmentFacade.getAppointmentsBySpecialistAndDateRange(this.specialist!.id, startDate, endDate),
        this.appointmentFacade.getAppointmentsByClientAndDateRange(clientId, startDate, endDate)
      ]);

      const bookedTimes = new Set([
        ...specialistAppointments.map(a => a.date.getTime()),
        ...clientAppointments.map(a => a.date.getTime())
      ]);
